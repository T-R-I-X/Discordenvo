...
export function output(DB, userId, flip, toAdd) {
  var Flip;
  var Input;

  if (!userId || !flip || !toAdd)
    throw new Error("coinFlip: missing parameters");

  Flip = flip.toLowerCase();
  if (Flip != "tails" && Flip != "heads")
    throw new Error("coinFlip: flip needs to be [tails  or heads]");

  if (!parseInt(toAdd))
    throw new Error("coinFlip: input needs to be a number");
  if (parseInt(toAdd) < 0)
    throw new Error("coinFlip: input needs to be greater than 0");
