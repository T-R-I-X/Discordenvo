...
export function output(DB, userId, toSubtract) {
  if (!userId)
    throw new Error("subtractBalance: missing userId");

  if (!toSubtract && toSubtract != 0)
    throw new Error("subtractBalance: missing toSubtract");

  if (!parseInt(toSubtract))
    throw new Error("subtractBalance: toSubtract needs to be a number");
