...
module.exports = {
  default: (DB,userId,toSet) => {
    if (!userId) throw new Error("setBalance: missing userId");

    if (!toSet && toSet > 0) throw new Error("setBalance: missing toSet");
    
    if (!parseInt(toSet)) throw new Error("setBalance: toSet needs to be a number");
