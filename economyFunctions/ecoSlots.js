...
export function output(DB, userId, toAdd, data) {
  var Input;

  if (!userId || !toAdd)
    throw new Error("ecoSlots: missing parameter");

  if (data.Rolls && !parseInt(data.Rolls))
    throw new Error("ecoSlots: data.Rolls needs to be a number");
  if (data.Emojis && !Array.isArray(data.Emojis))
    throw new Error("ecoSlots: data.Emojis needs to be an array");
  if (!data.Rolls)
    data.Rolls = 3;
  if (!data.Emojis)
    data.Emojis = ["🍎", "🍒", "🍉"];
  ...
  if (!parseInt(toAdd))
    throw new Error("ecoSlots: input needs to be a number");

  if (parseInt(toAdd) < 0)
    throw new Error("ecoSlots: toAdd needs to be greater than 0");
