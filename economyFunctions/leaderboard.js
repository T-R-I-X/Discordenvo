...
export function output(DB, S, data) {
  if (data.limit && !parseInt(data.limit))
    throw new Error("leaderboard: data.limit needs to be a number");
  if (data.limit)
    data.limit = parseInt(data.limit);

  if (data.filter && !data.filter instanceof Function)
    throw new Error("leaderboard: data.filter needs to be a function");
  if (!data.filter)
    data.filter = x => x;
  ...
  const ecoLeaderboardProm = new Promise(async (resolve, error) => {
    if (data.search) {
      ...
      if (output == -1)
        return resolve("leaderboard: user not found");
