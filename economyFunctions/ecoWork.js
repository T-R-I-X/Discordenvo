...
module.exports = {
  default: (DB,userId,data) => {
    if (!userId) throw new Error("ecoWork: missing parameters");

    if (data.jobs && !Array.isArray(data.jobs)) throw new Error("ecoWork: data.jobs is not an array");
    ...
    if (data.money && !parseInt(data.money)) throw new Error("ecoWork: data.money needs to be a number");
    if (data.money) data.money = parseInt(data.money);
    if (!data.money) data.money = Math.floor(Math.random() * 101);

    if (data.failurerate && !parseInt(data.failurerate)) throw new Error("Work Error: data.failurerate needs to be a number");
    if (data.failurerate) data.failurerate = parseInt(data.failurerate);
    if ((data.failurerate && data.failurerate < 0) || data.failurerate > 100) throw new Error("ecoWork: data.failurerate needs to be a number between 0-100");
